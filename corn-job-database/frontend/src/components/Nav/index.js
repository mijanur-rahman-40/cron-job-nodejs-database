            ...
            <div className="justify-between lg:flex items-center hidden" id="mobile-menu-2">
                <ul className="flex flex-col p-4 mt-4 bg-gray-50 rounded-lg border border-gray-100 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-white ">
                    {items.map((item, index) => (
                        <li>
                            <Link to='' className="block text-base py-2 pr-4 pl-3 text-gray-700 hover:bg-blue-700 rounded hover:text-white md:hover:text-blue-700 hover:md:bg-white md:p-0 transition">{item.text}</Link>
                        </li>
                    ))}
                </ul>
            </div>
            <div className="flex items-center">
                <div class="relative md:block hidden mr-3">
